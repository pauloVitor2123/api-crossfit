...
namespace SistemaCrossfit.Repositories
{
    public class StudentPointsRepository : IStudentPointsRepository
    {
        ...
        public async Task<List<StudentPointsWithExerciseNameDTO>> GetListByIdStudent(int idStudent)
        {
            List<StudentPointsWithExerciseNameDTO>? studentPoints = await _dbContext.StudentPoints.Where(x => x.IdStudent == idStudent).Join(_dbContext.Exercise,
            sp => sp.IdExercise,
            ex => ex.IdExercise,
            (sp, ex) => new StudentPointsWithExerciseNameDTO
            {
                IdStudentPoints = sp.IdStudentPoints,
                IdStudent = sp.IdStudent,
                IdExercise = sp.IdExercise,
                Points = sp.Points,
                ExerciseName = ex.Description,
        ...
        {
            List<StudentPointsWithExerciseNameDTO>? studentPoints = await GetListByIdStudent(idStudent);
